...
- `bash scripts/publish_dist.sh` — publish the committed `dist/` to the GitHub repository in `DIST_METADATA.repository_url` (clone, `rsync --delete`, push `main`). This is the dist publish path. Do not add a GitHub Actions workflow for it.
...
- `uv run python -m scripts.i_o_tables` — write `artifacts/startup-site/` (override with `--output-dir`): `startup-guide.csv`, `output-catalog.csv`, HTML pages, a workbook download, and excel-grapher's statement graph. Loads the cached graph the same way as the audit/burndown CLIs. Input domains come from series bindings via `cell_type_env_from_bindings` (`CellType` enum / interval / real_interval). Prints a Git-Bash-safe POSIX `--directory` serve hint. Do not add pandas; CSV/HTML use the stdlib. Run `uv run pytest tests/test_i_o_tables.py` after changing it.
- Author series bindings with the vendored excel-grapher skill at `.agents/skills/author-bindings` (Cursor also loads `.cursor/skills/author-bindings` if you copy it there). Pedagogical shapes live in `.agents/skills/author-bindings/assets/` (`catalog.example.yaml`, `measure-shards.example.yaml`). Do **not** emit four sidecar files from a generic catalog. Pipeline-wired `scripts.binding_resolution_audit` and `scripts.internal_binding_burndown` stay (cached `TARGETS` graph); they are not replaced by `excel-grapher bindings audit` / `burndown`, which build a graph from bound `data_range` targets.
- `uv run python -m src.extraction_pipeline --only-stage annotate` — splice LLM Google-style docstrings onto the inverted-tree `api.py` / `internals.py` (`src/inverted_tree_docstrings.py`). Requires a warm `validate.json` manifest. Cache: `.cache/inverted-tree-docstrings.json`. Uses `DOCSTRING_MODEL`.
- Export materialize seeds the FormulaEvaluator **series** graph into `dist/` from `templates/series-graph/` (`{package}/graph_*.py`, `assets/graph/`, `scripts/serve_graph_api.py`). Author `{package}/graph_schema.py` (`NODES`/`EDGES`); reference `templates/series-graph/examples/reference_graph_schema.py`. Document agent prompt requires a short landing-page embed (no long blurb).
