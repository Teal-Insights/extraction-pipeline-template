...
- `uv run python -m scripts.i_o_tables` — write `artifacts/startup-site/` (override with `--output-dir`): `startup-guide.csv`, `output-catalog.csv`, HTML pages, a workbook download, and excel-grapher's statement graph. Loads the cached graph the same way as the audit/burndown CLIs. Input domains come from series bindings via `effective_domain_annotations` (Literal / Between / RealBetween), with `CONSTRAINTS` overlay. Prints a Git-Bash-safe POSIX `--directory` serve hint. Do not add pandas; CSV/HTML use the stdlib. Run `uv run pytest tests/test_i_o_tables.py` after changing it.
