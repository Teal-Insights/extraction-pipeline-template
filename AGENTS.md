...
The live orchestrator is `extract → export → annotate → validate → document`. Warm extract also caches `derive_*_series` and `validate_series_bindings` payloads under `.cache/series-resolution/` and `.cache/bindings-validation/` (keyed from `graph_cache_key` plus a bindings fingerprint, like `.cache/series-derived/`), derived leaf classification / binding indexes / bound keys under `.cache/series-derived/` (also keyed from `graph_cache_key` plus bindings fingerprint, validation mode, and exemptions), codegen module texts under `.cache/codegen/` (keyed from the projection cache key, targets, docstring inputs, `paradigm`, and `excel-grapher` version). Annotate caches LLM Google-style docstrings in `.cache/inverted-tree-docstrings.json`. Dependency-graph cache keys do **not** fold bindings: the graph is a function of workbook + targets + constraints + blank_ranges + flags + `excel-grapher` version only. Leftover clustering/refactor scripts still use `.cache/clusters/` and `.cache/internals/` until those modules are deleted. Those directories are local/untracked; pytest redirects them via `tests/fixtures/test_state.py`. Pass `--no-cache` to bypass them.
...
Leftover clustering/refactor scripts (not called from the live orchestrator):
...
Clustering / schedule diagnostics (leftover; run compare before using leftover refactor scripts):

- `uv run python -m scripts.compare_cluster_variation_modes` — leftover: compare `independent` vs `dominant_key_only` series-fingerprint family counts (`--include` for diffs; `--clustering-mode` to override).
