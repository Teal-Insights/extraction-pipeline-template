...
> **Historical archive.** This literate notebook informed the template's early
> workflow and is not authoritative. Prefer [README.md](../README.md),
> [AGENTS.md](../AGENTS.md), and [technical_standard.md](../technical_standard.md)
> for current schema versions, cache keys, artifact paths, and stage layout.
> Figures that once lived under `docs/extraction-pipeline_files/` were removed
> with this archive note.
...
This rendering cell uses the docstring cache when available and calls OpenAI
...
Finally, we can generate the code and write a distributable package in `dist/tiny_dsa/`. Passing `series_bindings` and `bindings_workbook` emits records-shaped input setters such as `set_growth_baseline` and output compute functions such as `compute_output_baseline`.
...
# Scalar inputs: single-cell setters accept a bare scalar
set_country_name(ctx, "Litellia")
set_shock_year(ctx, 2)
set_shock_type(ctx, 1)  # growth shock

# Baseline trajectories: series setters accept a 1-D sequence of measure values
set_growth_baseline(ctx, [2.5, 2.4, 2.3, 2.2, 2.1])
set_interest_baseline(ctx, [5.2, 5.1, 5.0, 4.9, 4.8])
set_primary_balance_baseline(ctx, [-1.5, -1.0, -0.5, 0.0, 0.5])

# Shock table values: key-labeled series read most naturally as records
set_shock_magnitudes(
    ctx,
    [
...
User-guide runnable cells should follow this interaction model: import from `tiny_dsa.api`, create a context with `make_context()`, configure the scenario with `set_*` functions, then read results with `compute_*`. Single-cell setters accept a bare scalar; series setters accept a 1-D sequence of measure values, a tidy Polars DataFrame, or a list of records (use records when the series is keyed by a label such as `SHOCK_PARAMETER`).
...
4. On `NameError`, optionally call the LLM to rewrite only the failing cell (requires `OPENAI_API_KEY`, limited retries).
...
The automated entry point is `uv run -m src.extraction_pipeline`, which exports
the package (including codegen `internals.py`), runs
`refactor_internals_all_clusters()` to rewrite that file in place, and then
calls `run_documentation_pipeline()`.
...
2. Either choose the "optimal" packing of subgraphs (by deleting all nodes that only have to be substituted in one place), or, once we've identified all cartesian combinations of subgraphs that satisfy the incoming edge constraint, select the one that maximizes formula similarity across consolidated groups.
...
The proof of concept above demonstrates the *shape* of graph deduplication we want:
multi-node subgraph collapse with manifest lineage. For the reusable pipeline in
`src/`, we start with excel-grapher's built-in `OptimalCompression` instead of
hand-picked groups. That step is safe, provenance-aware compression over the
dependency graph: it forwards identity transits and inlines single-call-site
formula nodes when substitution is safe.

`OptimalCompression` requires edge provenance. Stage 2A therefore builds the
canonical `graph` with `capture_dependency_provenance=True`, matching
`src/extraction_pipeline.py`.

Similarity-aware packing from step 2 of the hypothesis workflow—enumerating
candidate subgraphs, ranking packings by size reduction, and choosing the
cluster-tightest formula family—is deferred as future work. For now,
`build_tiny_dsa_refactor_projection()` in `src/subgraph_projection.py` applies
`OptimalCompression` alone.
...
`OptimalCompression` keeps shared shock-active cells such as `Engine!C10` because
they have multiple dependents, while it removes output-layer identity transits
and inlines intermediate growth cells into the debt-to-GDP formulas.
...
`src/extraction_pipeline.py` exports the distributable package from this optimal
projection rather than the canonical graph. The public series API is unchanged;
internals are first emitted from the projected graph, then rewritten in place by
the refactor pass in the next section (and by
`refactor_internals_all_clusters()` when you run `uv run -m src.extraction_pipeline`).
The cell below writes the **codegen** export—the parallel `cell_*` bodies that
clustering and the LLM refactor consume as input.
...
Future work: replace or augment `OptimalCompression` with similarity-aware
subgraph packing that targets parallel, parameterizable formula families across
columns and years.

### Programmatic Python Deduplication and Refactoring

Graph compression gets us most of the way to a smaller export, but the generated
`dist/tiny_dsa/internals.py` can still contain parallel cell functions with the
same formula shape. The refactor pipeline now uses a graph-collapse model: each
eligible singleton or parallel cluster is collapsed into a semantic helper, every
caller is mechanically rewritten to the helper call for the specific callee
address, and the collapsed `cell_*` definitions are removed.
...
flowchart LR
  ...
  E --> F[apply_cluster_collapse]
  F --> G{more clusters?}
  G -->|yes| B
  G -->|no| H[apply_phase_c]
  H --> I[differential tests]
```

Steps in order:

1. **Cluster** — `cluster_graph_formulas(optimal_projection)` groups parallel
   formulas by canonicalized normalized formula shape.
2. **Refactor singletons** — known singleton cells such as `Inputs!B6` and
   `Engine!B9` become semantic helpers first, so later clusters can call them.
3. **Build context** — extract member sources, dependencies, call sites, binding
   key vocabulary, and expected `member_keys` from `(projection, cluster,
   internals_path)`.
4. **LLM call** — one cached JSON response per cluster. The model returns a
   helper function plus parameter and `member_keys` metadata; it does not return
   patches or per-call-site edits.
5. **Validate** — Pydantic parse plus AST, docstring, binding-key, and allowed
   call-target checks. Validation fails before any file write.
6. **Collapse** — insert the helper, replace every `xl_eval` or direct call to a
   collapsed member with the literal helper call for that callee address, remove
   the collapsed `cell_*` definitions, and update resolver dispatch for public
   addresses.
7. **Phase C** — prune leftover unreferenced thin wrappers and keep
   `_ADDRESS_DISPATCH` limited to addresses that still need resolver entry.
8. **Write and test** — persist `internals.py`, then run the exported-library
   differential suite.

#### LLM contract

`ClusterRefactorResponse` is keyed by binding concepts rather than column-letter
parameters:

- `helper_name` and `helper_source` define the semantic helper.
- `parameters[]` maps Python parameter names to binding concepts such as
  `TIME_PERIOD`.
- `member_keys[]` records one removed cell per entry with literal keys for that
  address.

The factory renders each member entry as a literal call such as
`shock_active(ctx, time_period=2)` and substitutes that call wherever the
specific callee address is referenced. Pass-through generalization happens in the
next LLM-authored helper, not in the mechanical rewriter.

#### Apply all clusters

`refactor_internals_all_clusters()` walks refactor units from
`compute_refactor_schedule()` in `src/refactor_order.py`. When the inter-family
cluster graph is acyclic, each fingerprint family is one unit; otherwise members
are scheduled in ready subsets that respect the cell-level dependency graph. It
re-extracts context after each collapse so later LLM calls see already-collapsed
upstream helpers.
...
for result in refactor_results:
    print(f"{result.helper_name}: {len(result.wrappers_applied)} cells collapsed")
print(f"Phase C pruned wrappers: {refactor_results[-1].phase_c_pruned}")
...
Unit tests in `tests/test_internals_refactor.py` exercise the same collapse plus
Phase C path via session-scoped fixtures in `tests/conftest.py` (codegen into a
temp directory, cached LLM responses, no reads or writes to `dist/`).

Then run the exported-library differential script (118 scenarios x 15 output
cells = 1,770 Excel-vs-Python comparisons):

```bash
uv run python tests/differential/differential_test_exported_library.py
```

Reports land in `data/differential/exported_library/parity_report.{csv,txt}`.
Syntax check (`ast.parse`), compilation (`compile(..., "exec")`), and Excel
parity together guard against semantic drift from parameterized helpers or
resolver dispatch.
