...
def test_load_canonical_api_example_reads_template() -> None:
    ...
    assert "do not call `make_context()`" in example
    assert "compute_example_output" in example
    ...
    assert "bare values" in example
    assert "canonical key order" in example
    assert "not records" in example
...
def test_build_section_prompt_includes_input_shape_guidance() -> None:
    ...
    assert "Do not call make_context()" in prompt
    assert "ctx = make_context()" not in prompt
