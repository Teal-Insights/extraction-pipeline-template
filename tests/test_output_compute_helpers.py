"""Output compute helpers (series-bindings schema 1.17.0).
...
def test_skill_catalog_declares_a_schema_version() -> None:
    ...
    major, minor, patch = (int(part) for part in catalog["schema_version"].split("."))
    assert (major, minor, patch) >= (1, 17, 0)


def test_skill_catalog_uses_empty_input_block() -> None:
    catalog = yaml.safe_load(SKILL_CATALOG.read_text(encoding="utf-8"))
    (series,) = catalog["inputs"]["series"]
    assert series["input"] == {}
