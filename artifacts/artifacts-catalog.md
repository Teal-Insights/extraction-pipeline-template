...
`caches` keys: `dependency-graph`, `bindings-validation`, `series-resolution`, `series-derived`, `projection`, `codegen`, `clusters`, `internals`. Live inverted-tree runs observe the first six; `clusters` and `internals` remain for leftover refactor-lab scripts until those modules are deleted.
