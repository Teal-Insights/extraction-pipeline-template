...
When dependency children are truncated, judge completeness only for visible references
you can verify from the formula text and listed children.
...
def collect_parent_audit_evidence(
    ...
    *,
    ...
    visible_dependencies = dependencies[:child_limit]
    ...
    if evidence.truncated_dependency_count:
        lines.extend(
            [
                ...
                f"{evidence.total_dependency_count}). Judge missing/spurious issues only "
                "for references you can verify from the parent formula and listed children.",
            ]
        )
    lines.extend(
        [
