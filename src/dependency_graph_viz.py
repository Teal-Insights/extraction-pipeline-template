...
def _graphviz_layout_setting() -> GraphvizLayoutSetting:
    ...
    if raw not in ("auto", "always", "never"):
        raise ValueError(
            f"GRAPHVIZ_LAYOUT must be auto, always, or never; got {raw!r}"
        )
    ...
    *,
    ...
    for key in graph.keys(order="workbook"):
        for dependency in graph.get_dependencies(key):
            ...
            dep_node = graph.get_node(dependency)
...
def build_structure_only_index_html(*, json_filename: str = DEFAULT_JSON_FILENAME) -> str:
    return f"""<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8" />
...
def write_dependency_graph_site(
    ...
    *,
    ...
    layout_mode: LayoutMode = (
        "graphviz_preset" if layout_enabled else "structure_only"
    )
    ...
    if layout_enabled:
        layout_stage = timer.stage("graphviz_layout") if timer is not None else nullcontext()
        with layout_stage:
