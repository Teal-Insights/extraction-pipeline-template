...
def render_validation_page(
    ...
    *,
    ...
    if evidence_kind == "formula_evaluator":
        ...
        rerun_block = """To re-run graph-oracle validation from the extraction repo:
...
```"""
        ...
        what_tested = (
            ...
            f"`compute_*` helpers, and compares those values against FormulaEvaluator "
            f"on the extraction graph."
        )
        report_path = "`tests/results/reference/parity_report.txt`"
        ...
        sweep = (
            f"The sweep covers **{total}** cell-level comparisons against the graph."
        )
        rerun_note = (
            "Rerun the exported-library differential from the extraction repo. "
            "Microsoft Excel is not required for FormulaEvaluator comparisons."
        )
