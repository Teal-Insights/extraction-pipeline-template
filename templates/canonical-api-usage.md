User-guide runnable cells should follow this interaction model: import `compute_*` functions from the generated package API module and pass a frozen `{Output}Inputs` bundle. Build the bundle with `{Output}Inputs.from_defaults(...)`, which fills `data.*_DEFAULT` and accepts leaf overrides as Python scalars or 1-D sequences. There is no evaluation context: do not call `make_context()` or `set_*`, and do not pass leaf kwargs directly into `compute_*`.
...
- Series overrides are 1-D sequences of measure values in the series' canonical key order. Pass exactly one value per key; omit optional arguments that already have defaults in the generated `data` module.
...
from {api_import_path} import ExampleOutputInputs, compute_example_output
